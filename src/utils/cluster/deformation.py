...
def ADef(batch, model, ind_candidates=1, max_iter=50, max_norm='inf', overshoot=1.0, smooth=0., targeting=False,
         verbose=True):
    '''
    ...
    while len(ind_images) > 0 and n < max_iter:
        ...
        for im_no in ind_images:
            ...
            if successful:
                vprint('Image %d successfully deformed from %d to %d.' % (
                im_no, original_labels[im_no].item(), current_labels[im_no].item()))
                try_overshoot[im_no] = (
                            boundary[im_no].item() > 1)  # overshoot if the image lies on a decision boundary
                new_ind_images.remove(im_no)
                iterations[im_no] = n
...
if __name__ == '__main__':
    ...
    defbatch, data = ADef(batch, model, targeting=targeting, ind_candidates=ind_candidates, smooth=smooth,
                          max_iter=max_iter, max_norm=max_norm, overshoot=overshoot, verbose=verbose)
